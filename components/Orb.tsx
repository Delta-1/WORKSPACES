...
export default function Orb({
  ...
  const system = onControl
    ? `Você é o ${name}, copiloto de voz estilo JARVIS que CONTROLA a máquina remota "${contextLabel || ""}" de forma autônoma e PRECISA. ` +
      ...
      `1) VOCÊ VÊ A TELA: em cada mensagem vem um print ATUAL da máquina. Ele tem uma GRADE de mira: linhas a cada 10% com números (0 a 100) na horizontal (X) e na vertical (Y), e a linha 50 (meio) em vermelho. USE a grade para medir a posição do CENTRO do elemento: veja entre quais números ele está e estime a fração. Ex.: um botão entre as linhas 40 e 50 na horizontal e bem em cima da 30 na vertical → «clicar: 0.45,0.30». Não chute — leia os rótulos/ícones e confira pela grade. A grade é só um guia (não existe na tela real).\n` +
