...
export default function Dock({
  ...
  return (
    // Quando o menu de apps está aberto (pinMode) a barra fica ACIMA do fundo
    // borrado (z-50) para a pessoa arrastar os apps pra cá, estilo inventário.
    <div className={`fixed bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 max-w-[96vw] ${pinMode ? "z-50" : "z-30"}`}>
      {pinMode && (
        <p className="text-center text-[11px] text-emerald-300/90 mb-1.5 select-none">
          Arraste os apps para cá para fixar na barra
        </p>
      )}
      <div
        onDragOver={(e) => {
      ...
      >
        ...
        {apps.map((app, i) => {
          ...
          return (
            <div key={app.id} className={`relative ${i >= 4 && !isActive ? "hidden sm:block" : "block"} shrink-0`}>
              <button
                draggable={pinMode}
        ...
        <button
          ...
          className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center cursor-pointer shrink-0 ${
            pinMode ? "bg-emerald-500/20 text-emerald-400" : "text-gray-400 hover:bg-white/10 hover:text-white"
          }`}
        >
          <Grid3x3 size={20} />
        </button>
